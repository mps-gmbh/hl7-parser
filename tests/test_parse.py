...
class TestParsing(unittest.TestCase):
    ...
    def test_successful_query_status(self):
        ...
        self.assertEqual(str(message.msa.acknowledgement_code), "AA")

    def test_unsuccessful_query_status(self):
        message = HL7Message(self.insuccessful_query_result)

        self.assertEqual(str(message.msa.acknowledgement_code), "AE")
        self.assertEqual(str(message.msa.text_message), "2018")

    def test_datetime(self):
        message = HL7Message(self.msg_string1)
        self.assertEqual(str(message.pid.datetime_of_birth), "19610615")
        delimiters = HL7Delimiters(*"|^~\\&")
        old = "18710826"
        dt = HL7Datetime(old, delimiters)
        self.assertEqual(str(dt), old)
        time = "19610615132733.0065"
        dt = HL7Datetime(time, delimiters)
        self.assertEqual(str(dt), time)

    def test_address(self):
        message = HL7Message(self.msg_string1)
        self.assertEqual(
            str(message.pid.patient_address),
    ...
    def test_simple_segments_parse(self):
        ...
        assert str(message.pid) == pid_seg
    ...
    def test_multiple_segments_parse(self):
        ...
        self.assertEqual(str(message.nk1[0]), lines[3])
        self.assertEqual(str(message.nk1[1]), lines[4])
        self.assertEqual(str(message.nk1[2]), lines[5])
    ...
    def test_trailing_segment_fields(self):
        ...
        self.assertEqual(str(pid.ssn_number_patient), "")
        self.assertEqual(str(pid), pid_string)
    ...
    def test_message_parse(self):
        ...
        assert str(expected_delimiters) == str(message.delimiters)

        # check patient data
        self.assertEqual("ADAM", str(message.pid.patient_name[0].given_name))
        self.assertEqual("ADAM", str(message.pid[4][0][1]))
        self.assertEqual("EVERYMAN", str(message.pid.patient_name[0].family_name))
        ...
        self.assertEqual(
            ...
            str(message.pid.patient_identifier_list[0].assigning_facility),
        )
        self.assertEqual(
            "123456789", str(message.pid.patient_identifier_list[1].id_number)
        )

        # check correct parsing of extended adress

        self.assertEqual(
            "HOME STREET", str(message.pid.patient_address[0].street_address.street_name)
        )
        self.assertEqual("Greensboro", str(message.pid.patient_address[0].city))
        self.assertEqual("Westeros", str(message.pid.patient_address[0].country))

        # check correct parsing of coded with exception

        self.assertEqual("Caucasian", str(message.pid.race[0].text))

        # check correct parsing of message type

        self.assertEqual("A01", str(message.header.message_type.trigger_event))
        self.assertEqual("ADT", str(message.header.message_type.message_code))

        # check correct parsing of processing type

        self.assertEqual("P", str(message.header.processing_id.processing_id))
        self.assertEqual("default", str(message.header.processing_id.processing_mode))

        # check correct parsing of version identifier

        self.assertEqual("2.7", str(message.header.version_id.version_id))
    ...
    def test_unknown_message_parse(self):
        ...
        self.assertEqual("45", str(message.lol[0]))
        self.assertEqual("blubber", str(message.lol[1][0][1]))
        self.assertEqual("blubber", str(message.lol[1][0][1]))
        self.assertEqual("hehe", str(message.lol[1][1][0]))
        self.assertEqual("dumdeldi", str(message.lol[2][0][1]))
        self.assertEqual("debbel", str(message.lol[2][1][1]))

    def test_mrg_message_parse(self):
        message = HL7Message(self.msg_mrg)
        self.assertEqual("A34", str(message.header.message_type.trigger_event))
        mrg = message.mrg
        ...
        self.assertEqual("3150123", str(mrg.prior_patient_identifier_list[0].id_number))
        self.assertEqual("Q3150123", str(mrg.prior_alternate_patient_id))
    ...
    def test_pv1_segment(self):
        ...
        self.assertEqual(str(segment.patient_class), "I")

        self.assertEqual(str(segment.assigned_patient_location.point_of_care), "2000")
        self.assertEqual(str(segment.assigned_patient_location.room), "2012")
        self.assertEqual(str(segment.assigned_patient_location.bed), "01")

        self.assertEqual(str(segment.attending_doctor[0].person_identifier), "004777")
        self.assertEqual(str(segment.attending_doctor[0].given_name), "AARON")
        self.assertEqual(str(segment.attending_doctor[0].family_name), "ATTEND")

        self.assertEqual(str(segment.attending_doctor[0].second_name), "A")

        self.assertEqual(str(segment.hospital_service), "SUR")

        self.assertEqual(
            str(segment.preadmit_number.effective_date.datetime), "2019-09-24 14:31:34"
        )
